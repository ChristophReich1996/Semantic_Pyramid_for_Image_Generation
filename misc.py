...
def get_masks_for_training(
        ...
        p_random_mask: float = 0.4, p_full_mask_on:float = 0.3) -> List[torch.Tensor]:
    '''
        ...
        # Random mask cases
            ...
            if len(mask_shape) > 2:
                ...
                if np.random.rand() < p_random_mask:
                    if np.random.rand() < p_full_mask_on:
                        random_mask_used = True
                        # Mask on
                        random_mask = torch.zeros(1, mask_shape[1], mask_shape[2], dtype=torch.float32)
                    else:
                        random_mask_used = True
